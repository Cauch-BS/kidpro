...
def find_level_for_target_mpp(slide_path: str, target_mpp: float) -> int | None:
    """
    ...
    x_resolution = float(slide.properties.get('tiff.XResolution') or 0)
    y_resolution = float(slide.properties.get('tiff.YResolution') or 0)
    ...
    if resolution_unit == 'centimeter':
        ...
        raise ValueError("Resolution unit is not in centimeters. Adjust the calculation accordingly.")

    # Check if MPP information is available
    if not mpp_x or not mpp_y:
        raise ValueError("Could not calculate MPP due to missing or invalid resolution information.")
        return None
