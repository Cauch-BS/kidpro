...
from kidpro.preprocessing.data import box_utils
...
class LoadROId(MapTransform):
    ...
    def _get_bounding_box(self, slide_obj: OpenSlide) -> box_utils.Box:
        ...
        level_size = slide_obj.level_dimensions[highest_level][::-1]
        slide_data, meta_data = self.image_reader.get_data(
            slide_obj,
            level=highest_level,
            size=level_size,
        )
        slide = np.asarray(slide_data)
